...
get_link_xcoor <- function(targ, ftab_cnv)
{
    # subset table
    x_coord <- sort(unlist(lapply(targ, function(x) range(ftab_cnv[grep(paste(x, "_", sep=""), rownames(ftab_cnv)),"bary"]))))    # define range of targets
...
plot_CNV_chr <- function(tab_cnv, tab_star, tab_tar, yli=c(-0.5, 2.5), centz=c(0.75,1.25), c_ex=.9, l_wd=.9)
#zcent: central zone, use the delimit the rounding area where alpha wil lbe rounded to 1
{
    # 1) define preliminary parameters
        # 1.0)
    sub_targ <- rownames(tab_cnv)
    gene <- collapse_elements(sub_targ[1], what=1:2)
    targ <- unique(sapply(sub_targ, collapse_elements))
    ...
    rg_coo <- c(min(tab_tar$startV2, na.rm=T), max(tab_tar$stopV2, na.rm=T))# define range of initial targets
    print("Range of the displayed marker:")
    print(range(rg_coo))
    plot(rg_coo, rep(1, 2), xlim=rg_coo, ylim=yli, type="n", main=gene, xlab="Chromosome coordinate (bp)", ylab="Alpha (CNV relative to standard)")
    ...
    if (length(targ)>1) 
        x_coord <- get_link_xcoor(targ, ftab_cnv)
