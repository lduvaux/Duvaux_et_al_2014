...
get_info_rking <- function(tab_best_obs, GLMtab_all1, v_races, varia=40){
    ...
    N_CDD_varia <- table(fetch_CDD[1:varia,])[3]
    ...
    # rks CDD in all the varia best subtargets
    rks_varia <- varia:1
    ind <- ifelse(info_CDD[1:varia,"CDD_best"]!="3_CpDup" | is.na(info_CDD[1:varia,"CDD_best"]), F, T)
    sum_rk_varia <- sum(rks_varia[ind])

    ll <- list(N_CDD=N_CDD, N_CDD_varia=N_CDD_varia, sum_rank_CDD=sum_rk, sum_rank_CDD_varia=sum_rk_varia, info_CDD=info_CDD)
}
