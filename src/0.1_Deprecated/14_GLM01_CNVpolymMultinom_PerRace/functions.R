...
plotmod_predic <- function(list_heat_map, GLMtab, trimmed, mat_layout = matrix(1:9, 3, 3, byrow=T))
{
	...
	for (ite in 1:length(list_heat_map)) {
		...
		for (j in 1:3)
		{
			...
			zp <- seq (0,1, length.out=6)
			image.plot(x_genes, y_exons, t(tab), xlab="Gene length (log)", ylab="Exon length (log)", main=paste(categ, polymo, sep="_"), lwd=0, breaks=brk, lab.breaks=names(brk), axis.args=list(at=zp, labels=zp), zlim=zl)
