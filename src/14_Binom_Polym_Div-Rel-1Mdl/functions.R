...
set_GLMtab <- function(tab0, NonTrim_only=F, covar)
{
	...
	if (covar=="LnExonLength") {
		tab <- with(tab0, data.frame(Polymorphic, Duplication, Phylog_lvl, Race, Gene, Family, trimmed=as.factor(trimmed), LnGeneLength=log(GeneLength), LnExonLength=log(TotExonLength)))
		bad <- which(is.na(tab$LnGeneLength) | is.na(tab$LnExonLength))}

	if (covar=="ratioLength") {
		tab <- with(tab0, data.frame(Polymorphic, Duplication, Phylog_lvl, Race, Gene, Family, trimmed=as.factor(trimmed), LnGeneLength=log(GeneLength), ratioLength=qlogis(ratioLength)))
